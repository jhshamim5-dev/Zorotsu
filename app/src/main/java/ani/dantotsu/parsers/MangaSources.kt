...
object MangaSources : MangaReadSources() {
    ...
    suspend fun init(fromExtensions: StateFlow<List<MangaExtension.Installed>>) {
        ...
        list = createParsersFromExtensions(initialExtensions) + listOf(
            Lazier({ WeebCentralParser() }, "Weeb Central (Inbuilt)"),
    ...
    fun performReorderMangaSources() {
        //remove the downloaded and inbuilt sources from the list to avoid duplicates
        list = list.filter { it.name != "Downloaded" && it.name != "Weeb Central (Inbuilt)" }
